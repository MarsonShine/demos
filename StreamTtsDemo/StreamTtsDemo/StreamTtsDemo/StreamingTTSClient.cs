...
{
    /// <summary>
    /// 火山引擎 TTS WebSocket 双向流式 API 客户端
    /// </summary>
            ...
            try
            {
                ...
                while (client.State == WebSocketState.Open)
                {
                    ...
                    if (result.MessageType == WebSocketMessageType.Binary)
                    {
                        ...
                        {
                            Console.WriteLine($"===> Event: {response.Optional.Event}");
                            ...
                            switch (response.Optional.Event)
                            {
                                ...
                                case EVENT_TTSResponse:
                                    if (response.Payload != null && response.Header?.MessageType == AUDIO_ONLY_RESPONSE)
                                    {
                                        ...
                                        Console.WriteLine($"===> 接收到音频数据块: {response.Payload.Length} 字节");
                                    }
                                    break;
                                ...
                                case EVENT_ConnectionFinished:
                                    ...
                                    Console.WriteLine("===> 数据接收完成，等待播放结束");
