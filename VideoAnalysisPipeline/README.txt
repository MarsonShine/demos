...
--video-mp4-muxer psp 用来写入截图中的 MPEG-4 (Sony PSP) / MSNV 封装标记。
--video-h264-profile main 与 --video-h264-level 3.1 用来导出 H.264 Main@L3.1；Main/High profile 会显式启用 CABAC。
--video-keyframe-interval-seconds 1 用来把最大关键帧间隔固定为 1 秒，改善 Android MediaPlayer/VideoView 的 seek 定点准确度。
--video-reference-frames 3 用来导出 3 个 H.264 参考帧，并关闭 B-pyramid 参考以保证 SPS 中恰好报告 3 个参考帧。
...
当你设置了任何视频导出参数（例如目标码率、H.264 profile/level、关键帧间隔、参考帧、分辨率、fps、AAC 参数、x264 preset）时，copy-source-video 这一步实际会变成重新编码导出；新版进度里会显示为 transcode-source-video，避免和真正的文件复制混淆。
